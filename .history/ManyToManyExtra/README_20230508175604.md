...
Como hemos comprobado, hemos pasado **de una asociación `@ManyToMany` bidireccional** a **una o asociaciones `@ManyToOne` + `@OneToMany`**. Por tanto, sería 

necesario excluir las listas en los métodos `equals`, `hashCode` y `toString` para evitar referencias circulares.